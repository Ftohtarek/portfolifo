...
class dbOperator implements dbDigram
{
    ...
    public function count(){
        $count = $this->cont->prepare("SELECT COUNT(*) FROM $this->tableName ");
        $count->execute();
    ...
    public function find($key, $value, $exceptId = 0)
    {
        $exceptIdSql = '0=?';
        if ($exceptId != 0) {
            $exceptIdSql = 'id !=?';
        }
        $rowNumber = $this->cont->prepare("SELECT * FROM $this->tableName WHERE $key=? AND $exceptIdSql LIMIT 1 ");
        if ($rowNumber->execute([$value, $exceptId])) {
